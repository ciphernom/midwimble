//! Hash-based signature chains, ported verbatim from midstate
//! `src/core/wots_simd.rs` (used by the recovery identities in `recovery.rs`).
//!
...
///
/// ```
/// # use midwimble::core::wots_simd::{SimdLevel, detect};
/// let level = detect();
...
impl SimdLevel {
    ...
    ///
    /// ```
    /// # use midwimble::core::wots_simd::SimdLevel;
...
///
/// ```
/// # use midwimble::core::wots_simd::{detect, SimdLevel};
/// let level = detect();
...
///
/// ```
/// # use midwimble::core::wots_simd::detected_level;
...
///
/// ```
/// # use midwimble::core::wots_simd::scalar_wots_chain;
/// let input = [0u8; 32];
///
...
///
/// ```
/// # use midwimble::core::wots_simd::{process_wots_batch, scalar_wots_chain};
/// # use midwimble::core::types::hash;
...
pub fn process_wots_batch(inputs: &[[u8; 32]], iters: &[usize]) -> Vec<[u8; 32]> {
    assert_eq!(
        inputs.len(),
        iters.len(),
        "inputs and iters length must match"
    );
    let n = inputs.len();
...
mod wasm_simd {
    ...
    /// dispatch path.
    pub unsafe fn compute_4way_wasm(starts: &[[u8; 32]; 4], iters: &[usize; 4]) -> [[u8; 32]; 4] {
...
mod avx2 {
    ...
    #[target_feature(enable = "avx2")]
    pub unsafe fn compute_8way_avx2(starts: &[[u8; 32]; 8], iters: &[usize; 8]) -> [[u8; 32]; 8] {
        let mut results = *starts;
        let max_iters = *iters.iter().max().unwrap_or(&0);
        if max_iters == 0 {
            return results;
        ...
        for i in 0..8 {
            hw[i] = _mm256_set_epi32(
                read_word(&starts[7], i),
                read_word(&starts[6], i),
                read_word(&starts[5], i),
                read_word(&starts[4], i),
                read_word(&starts[3], i),
                read_word(&starts[2], i),
                read_word(&starts[1], i),
                read_word(&starts[0], i),
            );
        }
...
mod neon {
    ...
    pub unsafe fn compute_4way_neon(starts: &[[u8; 32]; 4], iters: &[usize; 4]) -> [[u8; 32]; 4] {
        let mut results = *starts;
        let max_iters = *iters.iter().max().unwrap_or(&0);
        if max_iters == 0 {
            return results;
    ...
    fn test_process_wots_batch_correctness() {
        ...
        for i in 0..inputs.len() {
            ...
            assert_eq!(
                ...
                "Mismatch at index {i} with {} iterations",
                iterations[i]
            );
        }
    }
