...
impl MerkleMountainRange {
    ...
    /// The MMR of the first `n` leaves (midwimble addition, used to rebuild
    /// a fork-point state without rehashing the chain).
    ///
    /// An MMR only ever appends nodes in post-order, so the node array for
    /// `n` leaves is exactly the first `mmr_size(n)` nodes of any larger one.
    ///
    /// ```text
    ///   post:  result.L = L[0 .. min(n, #L)]
    /// ```
    pub fn truncated(&self, n: u64) -> Self {
        let n = n.min(self.leaf_count);
        Self {
            nodes: self.nodes.take(mmr_size(n) as usize),
            leaf_count: n,
        }
    }
    ...
    pub fn prove(&self, leaf_pos: u64) -> Result<MmrProof> {
        ...
        let peak_index = peak_positions
            ...
            .ok_or_else(|| anyhow::anyhow!("internal invariant violated: climbed to non-peak"))?;
...
impl UtxoAccumulator {
    ...
    ///   coins' = coins
    /// ```
    fn update_cache_for_bucket(&mut self, prefix: u16, bucket_coins: &[[u8; 32]], is_v2: bool) {
        debug_assert!(
            bucket_coins.windows(2).all(|w| w[0] < w[1]),
...
mod tests {
    ...
    #[test]
    fn mmr_truncated_matches_shorter_history() {
        let leaves: Vec<[u8; 32]> = (0..37u64).map(|i| hash(&i.to_le_bytes())).collect();
        let mut full = MerkleMountainRange::new();
        for l in &leaves {
            full.append(l, true);
        }
        for n in [0usize, 1, 2, 3, 8, 21, 37] {
            let mut short = MerkleMountainRange::new();
            for l in &leaves[..n] {
                short.append(l, true);
            }
            let cut = full.truncated(n as u64);
            assert_eq!(cut.leaf_count(), n as u64);
            assert_eq!(cut.root(true), short.root(true), "n = {n}");
        }
    }
    ...
    fn utxo_deserialize_then_rebuild_recovers_root() {
        ...
        for_each_mode(|v2| {
            ...
            let mut restored: UtxoAccumulator = bincode::deserialize(&bytes).expect("deserialize");
