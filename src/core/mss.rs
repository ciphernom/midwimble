//! Merkle signature scheme over WOTS, ported from midstate `src/core/mss.rs`.
//! One change: the key-generation checkpoint file is keyed by a fingerprint of
//! the seed, so an interrupted generation for one seed can never be resumed
//! into the key of another (upstream uses `mss_h{height}.checkpoint` for all
//! seeds, which yields a key whose signatures do not verify).
//!
...
pub fn keygen(master_seed: &[u8; 32], height: u32) -> Result<MssKeypair> {
    ...
    #[cfg(not(target_arch = "wasm32"))]
    {
        ...
        let fingerprint = hex::encode(
            &crate::core::types::hash_domain(b"midwimble.mss.checkpoint.v1", &[master_seed])[..8],
        );
        let checkpoint_path =
            std::path::PathBuf::from(format!("mss_h{}_{}.checkpoint", height, fingerprint));
            ...
            if let Ok(file) = std::fs::File::open(&checkpoint_path) {
                ...
                if let Ok(saved_leaves) = bincode::deserialize_from::<_, Vec<[u8; 32]>>(reader) {
                    if saved_leaves.len() == num_leaves as usize {
                        ...
                        tracing::info!(
                            "Loaded checkpoint for height {}. Resuming from {} leaves.",
                            height,
                            leaves_completed
                        );
                    }
        ...
        let total_finished =
            std::sync::Arc::new(std::sync::atomic::AtomicUsize::new(leaves_completed));
        let chunk_size = 4096;
        ...
        for chunk_start_idx in (leaves_completed..num_leaves as usize).step_by(chunk_size) {
            ...
            let rate = if elapsed > 0.0 {
                leaves_this_session as f64 / elapsed
            } else {
                0.0
            };
            let eta = if rate > 0.0 {
                (num_leaves as usize - count) as f64 / rate
            } else {
                0.0
            };
...
impl MssKeypair {
    ...
    pub fn peek_next_leaf(&self) -> u64 {
        self.next_leaf
    }

    /// Force leaf index (recovery only — reuse breaks security).
    pub fn set_next_leaf(&mut self, idx: u64) {
        self.next_leaf = idx;
    }
}
...
pub fn verify(sig: &MssSignature, message: &[u8; 32], master_pk: &MasterPublicKey) -> bool {
    ...
    let capacity = 1u64
        .checked_shl(sig.auth_path.len() as u32)
        .unwrap_or(u64::MAX);
...
impl MssSignature {
    ...
    pub fn from_bytes(data: &[u8]) -> Result<Self> {
        ...
        let leaf_index = u64::from_le_bytes(
            data[0..8]
                .try_into()
                .map_err(|_| anyhow::anyhow!("leaf_index slice error"))?,
        );
        ...
        let auth_len = u32::from_le_bytes(
            data[ao..ao + 4]
                .try_into()
                .map_err(|_| anyhow::anyhow!("auth_len slice error"))?,
        ) as usize;
...
mod tests {
    ...
    fn sig_size_reasonable() {
        ...
        assert_eq!(
            sig.auth_path.len(),
            2,
            "height-2 key has a 2-node auth path"
        );
    }
    #[test]
...
mod recovery_tests {
    ...
    fn recover_master_pk_matches_verify() {
        ...
        for i in 0..16u8 {
            ...
            let sig = kp.sign(&msg).unwrap();
            assert!(
                verify(&sig, &msg, &master_pk),
                "setup: signature must verify"
            );
            assert_eq!(
